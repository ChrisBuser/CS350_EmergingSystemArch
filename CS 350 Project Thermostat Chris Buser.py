...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        ## lights logic placed in updateLights() to keep code maintainable and avoid duplication
        self.updateLights()
    ...
    def on_exit_heat(self):
        ## red light only used in heat state, so turn it off when leaving heat state
        redLight.off()
    ...
    def on_exit_cool(self):
        ## blue light only used in cool state, so turn it off when leaving cool state
        blueLight.off()
    ...
    def processTempStateButton(self):
        ...
        ## iterate through the cycle of states
        self.cycle()
    ...
    def processTempIncButton(self):
        ...
        ## lights will be updated immediately after increasing setPoint to reflect change
        self.setPoint += 1
        self.updateLights()
    ...
    def processTempDecButton(self):
        ...
        ## lights will be updated immediately after decreasing setPoint to reflect change
        self.setPoint -= 1
        self.updateLights()
    ...
    def updateLights(self):
        ...
        if(DEBUG):
            print(f"State: {next(iter(self.configuration))}")
            print(f"SetPoint: {self.setPoint}")
            print(f"Temp: {temp}")

        # Determine visual identifiers

        ## current state is heat
        if next(iter(self.configuration))  == 'heat': #next(iter(sm.configuration)) 
            ## pulse red light to indicate system is actively heating
            if temp < self.setPoint:
                redLight.pulse(1, 1)
            ## solid red light to indicate setPoint has been reached
            else:
                redLight.on()

        ## current state is cool        
        elif next(iter(self.configuration)) == 'cool':
            ## pulse blue light to indicate system is actively cooling
            if temp > self.setPoint:
                blueLight.pulse(1, 1)
            ## solid blue light to indicate setPoint has been reached
            else:
                blueLight.on()  

        ## current state is off, turn all lights off        
        elif next(iter(self.configuration)) == 'off':
            redLight.off()
            blueLight.off()
    ...
    def setupSerialOutput(self):
        ## string with current state, current temperature, and set point
        output = f"{next(iter(self.configuration))},{floor(self.getFahrenheit())},{self.setPoint}"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ## string with current time in the format of Month Day Hour:Minute:Second
            lcd_line_1 = datetime.now().strftime('%b %d  %H:%M:%S\n')
    
            ## Setup Display Line 2
            if(altCounter < 6):
                ## string with current temperature
                lcd_line_2 = f"Current: {floor(self.getFahrenheit())}F"
    
                altCounter = altCounter + 1
            else:
                ## string with current state and setPoint
                lcd_line_2 = f"{next(iter(self.configuration))}, Set {self.setPoint}F"
            ...
            if((counter % 30) == 0):
                ## send info about current state state and temperature to over the serial port
...
##
## Button color on prototype is yellow
##
